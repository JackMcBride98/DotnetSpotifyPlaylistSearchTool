...
backed up by a backed up by an ACM SSL/TLS certificate for HTTPS termination at the ALB.
...
NAT gateway, but these are expensive to run, so we have it on public and access setup with a Security Group.
...
to point to the latest uploaded docker images, these are tagged by commit sha, we then run these tasks, first the database migrations
as a one off task and then the deploy of the API task to the API ECS service, which handles zero downtime deployment of this.
